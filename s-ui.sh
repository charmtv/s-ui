...
update() {
    confirm "此操作会强制重装最新版本，现有数据不会丢失。是否继续？" "n"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/charmtv/s-ui/main/install.sh"
...
uninstall() {
    ...
    echo ""
    echo -e "卸载完成。如需删除管理脚本，请退出后执行 ${green}rm -f /usr/bin/s-ui${plain}。"
    echo ""
...
reset_admin() {
    echo "不建议将管理员账号和密码恢复为默认值。"
    confirm "确定要恢复默认管理员信息吗？" "n"
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/s-ui -N --no-check-certificate https://raw.githubusercontent.com/charmtv/s-ui/main/s-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "下载脚本失败，请检查 GitHub 网络连接。"
        before_show_menu
    else
        chmod +x /usr/bin/s-ui
        LOGI "管理脚本更新成功，请重新运行 s-ui。" && exit 0
    fi
}
...
show_enable_status() {
    check_enabled $1
    if [[ $? == 0 ]]; then
        echo -e "${1} 开机自启：${green}已启用${plain}"
    else
        echo -e "${1} 开机自启：${red}未启用${plain}"
    fi
}
...
disable_bbr() {
    ...
    if [[ $(sysctl net.ipv4.tcp_congestion_control | awk '{print $3}') == "cubic" ]]; then
        echo -e "${green}已将 BBR 切换为 CUBIC。${plain}"
    else
...
enable_bbr() {
    ...
    arch | manjaro | parch)
        ...
        ;;
    *)
        echo -e "${red}不支持当前操作系统，请手动安装所需软件包。${plain}\n"
        exit 1
        ;;
    esac
...
ssl_cert_issue() {
    if ! command -v ~/.acme.sh/acme.sh &>/dev/null; then
        echo "未找到 acme.sh，准备自动安装。"
        install_acme
        if [ $? -ne 0 ]; then
            LOGE "acme.sh 安装失败，请查看日志。"
            exit 1
        fi
    fi
    case "${release}" in
    ...
    *)
    ...
    if [ ${currentCert} == ${domain} ]; then
        ...
        LOGE "系统中已存在该域名证书，无法重复申请。当前证书信息："
        LOGI "$certInfo"
        exit 1
    else
        LOGI "域名检查完成，准备申请证书。"
    ...
    local WebPort=80
    read -p "请输入验证端口（默认 80）：" WebPort
    if [[ ${WebPort} -gt 65535 || ${WebPort} -lt 1 ]]; then
        LOGE "端口 ${WebPort} 无效，将使用默认端口。"
    fi
    LOGI "将使用端口 ${WebPort} 申请证书，请确保该端口已放行。"
...
ssl_cert_issue_CF() {
    ...
    case $choice in
        1|2)
            ...
            LOGI "3. 已通过 Cloudflare 解析到当前服务器的域名"
            LOGI "4. 证书默认安装到 /root/cert"
            confirm "确认继续？" "y"
            if [ $? -eq 0 ]; then
                if ! command -v ~/.acme.sh/acme.sh &>/dev/null; then
                    echo "未找到 acme.sh，准备自动安装。"
                ...
                LOGD "请设置注册邮箱："
                read -p "请输入注册邮箱：" CF_AccountEmail
                LOGD "当前注册邮箱：${CF_AccountEmail}"

                ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
                if [ $? -ne 0 ]; then
                    LOGE "设置 Let's Encrypt 为默认证书机构失败，脚本退出。"
                    exit 1
                fi
...
show_usage() {
    ...
    echo -e "s-ui status       - 查看服务状态"
    echo -e "s-ui enable       - 启用开机自启"
    echo -e "s-ui disable      - 禁用开机自启"
    echo -e "s-ui log          - 查看日志"
    echo -e "s-ui update       - 更新面板"
    echo -e "s-ui install      - 安装面板"
    echo -e "s-ui uninstall    - 卸载面板"
    echo -e "s-ui help         - 查看命令帮助"
    echo -e "------------------------------------------"
}
...
show_menu() {
  ...
  ${green}1.${plain} 安装面板
  ${green}2.${plain} 更新面板
  ${green}3.${plain} 安装指定版本
  ${green}4.${plain} 卸载面板
  -----------------------------
  ${green}5.${plain} 恢复默认管理员信息
  ${green}6.${plain} 设置管理员信息
  ${green}7.${plain} 查看管理员信息
  -----------------------------
  ${green}8.${plain} 恢复默认面板设置
  ...
  -----------------------------
  ${green}18.${plain} BBR 管理
  ${green}19.${plain} SSL 证书管理
  ${green}20.${plain} Cloudflare 证书管理
  -----------------------------"
    show_status s-ui
